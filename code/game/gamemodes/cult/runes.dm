...
/obj/effect/rune/teleport/invoke(var/key)
	var/mob/living/user = usr
	var/allrunesloc[]
	allrunesloc = new/list()
	var/index = 0
//	var/tempnum = 0
	for(var/obj/effect/rune/teleport/R in global.runes)
		if(R == src)
			continue
		if(R.word3 == key && R.z != 2)
			index++
...
/obj/effect/rune/itemport/invoke(var/key)
//			var/allrunesloc[]
//			allrunesloc = new/list()
//			var/index = 0
//	var/tempnum = 0
	...
	var/mob/living/user = usr
	for(var/obj/effect/rune/itemport/R in global.runes)
		if(R == src)
			continue
		if(R.word3 == key)
			IP = R
	...
	if(runecount >= 2)
		...
		if (istype(user, /mob/living))
			user.take_overall_damage(5, 0)
		del(src)
	if(user.loc==src.loc || runecount<1)
		return fizzle()
...
/obj/effect/rune/tomesummon/invoke()
	if(istype(src,/obj/effect/rune))
...
/obj/effect/rune/proc/check_equip()
	if(ishuman(usr))
...
/obj/effect/rune/tearreality/invoke()
	var/cultist_count = 0
...
/obj/effect/rune/emp/invoke(var/U,var/range_red) //range_red - var which determines by which number to reduce the default emp range, U is the source loc, needed because of talisman emps which are held in hand at the moment of using and that apparently messes things up -- Urist
	if(istype(src,/obj/effect/rune))
...
/obj/effect/rune/raise/invoke()
	...
	find_sacrifice:
		for(var/obj/effect/rune/raise/R in global.runes)
...
/obj/effect/rune/obscure/invoke(var/rad)
	var/S=0
...
/obj/effect/rune/ajourney/invoke() //some bits copypastaed from admin tools - Urist
	if(usr.loc==src.loc)
...
/obj/effect/rune/manifest/invoke()
...
/obj/effect/rune/talisman/invoke()//only hide, emp, teleport, silence, blind and tome runes can be imbued atm
	...
	for(var/obj/effect/rune/R in orange(1,src))
		if(R==src)
			continue
		switch(R.type)
			if(/obj/effect/rune/teleport)  //teleport
				T = new(src.loc)
				T.imbue = "[R.word3]"
				T.info = "[R.word3]"
				imbued_from = R
				break
			if(/obj/effect/rune/cultsummon) //summon cult
				T = new(src.loc)
				T.imbue = "sumcult"
				T.uses = 2
				imbued_from = R
				break
			if(/obj/effect/rune/tomesummon) //tome
				T = new(src.loc)
				T.imbue = "newtome"
				imbued_from = R
				break
			if(/obj/effect/rune/emp) //emp
				T = new(src.loc)
				T.imbue = "emp"
				T.uses = 3
				imbued_from = R
				break
			if(/obj/effect/rune/obscure) //conceal
				T = new(src.loc)
				T.imbue = "conceal"
				T.uses = 2
				imbued_from = R
				break
			if(/obj/effect/rune/armor) //armor
				T = new(src.loc)
				T.imbue = "armor"
				imbued_from = R
				break
			if(/obj/effect/rune/revealrunes) //reveal
				T = new(src.loc)
				T.imbue = "revealrunes"
				T.uses = 3
				imbued_from = R
				break
			if(/obj/effect/rune/silence) //silence
				T = new(src.loc)
				T.imbue = "silence"
				imbued_from = R
				break
			if(/obj/effect/rune/blind) //blind
				T = new(src.loc)
				T.imbue = "blind"
				imbued_from = R
				break
			if(/obj/effect/rune/communicate) //communicat
				T = new(src.loc)
				T.imbue = "communicate"
				T.uses = 4
				imbued_from = R
...
/obj/effect/rune/communicate/invoke()
...
/obj/effect/rune/sacrifice/invoke()
...
/obj/effect/rune/proc/sac_grant_word()	//The proc that which chooses a word rewarded for a successful sacrifice, sacrifices always give a currently unknown word if the normal checks pass
...
/obj/effect/rune/proc/transmit_words()	//transmits words to all cultists memory
...
/obj/effect/rune/proc/check_stone(var/mob/living/carbon/human/H)		//check to see if user has empty soulstone on himself
	if(ishuman(H))
...
/obj/effect/rune/revealrunes/invoke(var/rad,var/obj/W as obj)
	var/S=0
	for(var/obj/effect/rune/R in orange(rad,src))
		if(R!=src)
			R.invisibility=0
...
/obj/effect/rune/freedom/invoke()
	var/mob/living/user = usr
...
/obj/effect/rune/cultsummon/invoke()
	var/mob/living/user = usr
	if(istype(src,/obj/effect/rune))
		if(active)
...
/obj/effect/rune/silence/invoke()
	if(istype(src,/obj/effect/rune))
		var/affected = 0
...
/obj/effect/rune/bloodboil/invoke() //cultists need at least one DANGEROUS rune. Even if they're all stealthy.
/*
	var/list/mob/living/carbon/cultists = new
	for(var/datum/mind/H in ticker.mode.cult)
		if (istype(H.current,/mob/living/carbon))
...
/obj/effect/rune/burningblood/invoke()
	var/culcount = 0
	for(var/mob/living/carbon/C in orange(1,src))
		if(iscultist(C) && !C.stat)
			culcount++
		...
		for(var/obj/effect/rune/R in global.runes)
			if(R.blood_DNA == src.blood_DNA)
				for(var/mob/living/M in orange(2,R))
					...
					T.hotspot_expose(700,125)
		for(var/obj/effect/decal/cleanable/blood/B in global.runes)
...
/obj/effect/rune/runestun/invoke(var/mob/living/T as mob)
...
/obj/effect/rune/shadow/invoke()
	...
	if(A.shadow)	//return if there is already a rune working
		...
		return
	var/rune_count
	for(var/obj/effect/rune/shadow/R in global.runes)
		if(R.active)
			rune_count++
	var/mob/living/user = usr
	if(rune_count > 3)
...
/obj/effect/rune/shadow/Del()
	shadow_remove()		//exit shadow rune
	..()

/obj/effect/rune/proc/shadow_remove()
	var/area/A = get_area_master(src)
